...
def get_file_hash(file_name):
    """Dosyanın SHA-256 hash'ini hesaplar ve PyInstaller temp klasörünü dikkate alır."""
    ...
    if not os.path.exists(file_path):
        print(f"Hata: {file_name} bulunamadı! Bu dosya PyInstaller içinde olmayabilir.")
...
def load_remote_version():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Hata! GitHub'daki version.json alınamadı. Status Code: {response.status_code}")
            return None
...
def download_updates():
    ...
    try:
        ...
        new_version = {
            "version": remote_version["version"], "files": local_version["files"].copy()}
