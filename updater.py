...
def load_local_version():
    ...
    try:
        ...
        log_and_print("Uyarı: Yerel version.json bulunamadı, varsayılan değerlere geçiliyor.")
...
def load_remote_version():
    ...
    try:
        ...
        log_and_print(f"İndirme hatası (version.json): {str(e)}")
        return None
...
def download_updates():
    ...
    try:
        ...
        # Önceki dosyaları koruyarak 'files' içinde sadece değişenleri güncelleyeceğiz
        new_version = {
        ...
        for file_name, remote_hash in remote_version["files"].items():
            ...
            if remote_hash != local_hash:
                ...
                log_and_print(f"İndiriliyor: {file_name}")
                ...
                if response.status_code == 200:
                    ...
                    log_and_print(f"{file_name} başarıyla güncellendi.")
                else:
                    log_and_print(f"Dosya indirilemedi: {file_name}, Hata kodu: {response.status_code}")

        if not guncel_mi:
            # Güncellenmiş version.json'ı kaydet
            with open(VERSION_FILE, "w", encoding="utf-8") as f:
                json.dump(new_version, f, indent=4)
            log_and_print("version.json güncellendi, yeni SHA değerleri kaydedildi.")
            return False
        else:
            log_and_print("Program zaten güncel, güncelleme yapılmadı.")
            return True

    except Exception as e:
        log_and_print(f"İndirme/güncelleme sırasında hata oluştu: {str(e)}")
        return False
